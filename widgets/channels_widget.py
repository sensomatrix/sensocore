...
class Channels(QListView):
    ...
    def create_menu(self, pos):
        ...
        if len(self.selectedIndexes()) == 2:
            index_1 = self.selectedIndexes()[0]
            index_2 = self.selectedIndexes()[1]

            action = menu.addAction("Cross Correlation")
            action.triggered.connect(lambda: self.model().cross_correlation(index_1, index_2))
